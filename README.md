...
规范代码变量命名 + UI/日志/绘图字符串审查 + 文档标题（Caption）命名审查。内置 **600+ 条**中英文术语词典，覆盖通用编程、雷达/信号处理/标校（含目标探测雷达，天气雷达，民航多普勒，激光测风雷达，气溶胶激光雷达）、气象/大气科学 领域。
...
语法正确的中英文混写 **容许**，仅报格式和术语规范问题：

| 检测项 | 示例 → 建议 |
|--------|------------|
| 缺空格 | `'功率(dB)'` → `'功率 (dB)'` |
| 冒号后缺空格 | `'参数:%.1f'` → `'参数: %.1f'` |
| 格式符前后缺空格 | `'处理%d个'` → `'处理 %d 个'` |
| 英文缩写大小写 | `prf`, `snr` → `PRF`, `SNR`（对照词典）|
| 英文 Title Case | `title('doppler spectrum')` → `title('Doppler Spectrum')` |
| 跨文件术语一致性 | "地物杂波" vs "地面杂波" → 统一 |

**可审查的函数：** `fprintf`、`printf`、`console.log`、`title`、`xlabel`、`ylabel`、`legend`、`sgtitle`、`error`、`warning`、`alert` 等。
...
**英文 Title Case 规则：** 实词（名/动/形/副）大写，冠词/介词/连词小写，**首词和末词**无论词性必大写。

## 用法命令

| 命令 | 功能 |
|------|------|
| `/variable-naming review [路径]` | 审查代码变量命名 |
| `/variable-naming fix [路径]` | 审查并自动修复变量命名 |
...
| `/variable-naming ui review [路径]` | 审查 UI/日志/绘图字符串 |
| `/variable-naming ui fix [路径]` | 审查并修复 UI 字符串 |

## 术语词典

内置 **600+ 条**中英文术语，覆盖 8 大章节：
